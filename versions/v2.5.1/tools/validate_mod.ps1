...
foreach ($item in @(
	@($descriptorPath, 'version="2.5.1"', 'supported_version="4.4.*"'),
	@($launcherPath, 'version="2.5.1"', 'path="mod/rogue_servitor_enhanced_v2"')
)) {
...
$required = @{
	...
	'common\economic_categories\rse_economic_categories.txt' = @(
		...
		'key = planet_artisans', 'key = planet_enforcers',
...
$forbidden = @{
	...
	'job_patrol_drone_add\s*=\s*-' = 'Economic-category inheritance must replace negative Hunter-Seeker job conversion.'
}
...
foreach ($key in @('trait_rse_organic_simulator_effect', 'trait_rse_intelligence_simulator_effect', 'trait_rse_desire_simulator_effect')) {
	if ($traitText.Contains($key)) { $errors.Add("Simulator trait effect must remain hidden from its trait tooltip: $key") }
}
...
foreach ($needle in @(
	'has_country_flag = transformation_nanotech', 'nanites = @rse_maid_refinery_output',
	'has_country_flag = transformation_modularity', 'sr_living_metal = @rse_maid_refinery_output',
	'has_country_flag = transformation_virtuality', 'energy = @rse_maid_path_energy_output',
	'has_ascension_perk = ap_interdimensional_processing', 'sr_zro = @rse_maid_zro_output',
	'has_technology = tech_mine_dark_matter', 'sr_dark_matter = @rse_maid_dark_matter_output',
	'trade = @rse_maid_workshop_trade'
)) {
	if (-not $mechanicalMaidBlock.Value.Contains($needle)) { $errors.Add("Mechanical Maid V2.5 production line missing: $needle") }
}
...
if (Test-Path -LiteralPath $GamePath) {
	...
	foreach ($key in @(
		...
		'ap_interdimensional_processing', 'tech_mine_dark_matter',
