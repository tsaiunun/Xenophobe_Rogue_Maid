...
foreach ($item in @(
	@($descriptorPath, 'version="2.7.1"', 'supported_version="4.4.*"'),
	@($launcherPath, 'version="2.7.1"', 'path="mod/rogue_servitor_enhanced_v2"')
)) {
...
$required = @{
	...
	'common\scripted_variables\rse_variables.txt' = @(
		...
		'@rse_home_service_assembly = 0.20', '@rse_home_service_tradition_assembly = 0.10',
	...
	'common\script_values\rse_script_values.txt' = @(
		...
		'rse_patrol_drone_conversion_units', 'base = this.rse_patrol_drone_slots_snapshot', 'divide = 100'
	)
	'common\traits\rse_traits.txt' = @(
		...
		'triggered_planet_pop_group_modifier_for_species = {', 'divide_over_pop_groups = no',
		'bonus_pop_growth = @rse_organic_simulator_assembly_per_pop',
		'logistic_growth_mult = @rse_desire_simulator_growth_per_pop'
	)
	'common\buildings\rse_buildings.txt' = @(
		...
		'job_patrol_drone_add = -100', 'job_rse_home_service_add = 100',
		'mult = value:rse_patrol_drone_conversion_units',
	...
	'events\rse_route_events.txt' = @(
		...
		'which = rse_home_service_assembly_output value = 10',
	...
	'common\static_modifiers\rse_static_modifiers.txt' = @(
		...
		'fake_pop_growth_mod = 1', 'bonus_pop_growth = 1'
	)
	...
	'events\rse_conversion_events.txt' = @(
		...
		'id = rse_conversion.3', 'trigger = num_assigned_jobs', 'trigger = free_jobs_of_type',
		'job = patrol_drone', 'set_carrier_flag = rse_patrol_drone_conversion_active'
	)
...
$forbidden = @{
	...
	'include_deprioritized_jobs\s*=' = 'free_jobs_of_type has no include_deprioritized_jobs parameter in vanilla 4.4.'
}
...
foreach ($pair in @(
	@('trait_rse_organic_simulator', 'trait_rse_organic_simulator_effect', 'planet.value:rse_organic_simulator_cap_multiplier'),
	@('trait_rse_desire_simulator', 'trait_rse_desire_simulator_effect', 'planet.value:rse_desire_simulator_cap_multiplier')
)) {
	$block = [regex]::Match($traitText, "(?ms)^$($pair[0])\s*=\s*\{.*?^\}")
	if (-not $block.Success) { $errors.Add("Simulator trait block missing: $($pair[0])"); continue }
	foreach ($needle in @("custom_tooltip = $($pair[1])", 'triggered_planet_pop_group_modifier_for_species = {', 'divide_over_pop_groups = no', "mult = $($pair[2])")) {
		if (-not $block.Value.Contains($needle)) { $errors.Add("Simulator direct per-pop effect missing from $($pair[0]): $needle") }
	}
}
...
if ($routeMaintenanceText -match 'add_modifier\s*=\s*\{\s*modifier\s*=\s*rse_(?:organic|desire)_simulator_scaled') {
	$errors.Add('Organic and Desire Simulator effects must not recreate visible monthly planet modifiers.')
}
$staticTextForSimulator = [IO.File]::ReadAllText((Join-Path $ModPath 'common\static_modifiers\rse_static_modifiers.txt'), [Text.Encoding]::UTF8)
foreach ($key in @('rse_organic_simulator_scaled', 'rse_desire_simulator_scaled')) {
	$block = [regex]::Match($staticTextForSimulator, "(?ms)^$key\s*=\s*\{(.*?)^\}")
	if (-not $block.Success -or $block.Groups[1].Value -match '[A-Za-z0-9_]\s*=') {
		$errors.Add("Legacy simulator modifier must remain an effect-free save-compatibility shell: $key")
	}
}
