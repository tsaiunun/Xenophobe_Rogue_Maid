...
foreach ($item in @(
	@($descriptorPath, 'version="2.5.0"', 'supported_version="4.4.*"'),
	@($launcherPath, 'version="2.5.0"', 'path="mod/rogue_servitor_enhanced_v2"')
)) {
...
$required = @{
	...
	'common\buildings\rse_buildings.txt' = @(
		...
		'building_rse_maid_base', 'job_patrol_drone_add = -200', 'job_patrol_drone_add = -500',
		'job_artisan_drone_add = -200', 'building_rse_maid_automation_coordination',
...
if (Test-Path -LiteralPath $GamePath) {
	...
	foreach ($key in @(
		...
		'planet_defense_armies_add', 'job_artisan_drone_add', 'job_patrol_drone_add'
	)) {
