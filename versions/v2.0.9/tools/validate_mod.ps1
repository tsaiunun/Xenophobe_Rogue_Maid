...
foreach ($item in @(
	@($descriptorPath, 'version="2.0.9"', 'supported_version="4.4.*"'),
	@($launcherPath, 'version="2.0.9"', 'path="mod/rogue_servitor_enhanced_v2"')
)) {
...
$required = @{
	...
	'common\policies\rse_policies.txt' = @('rse_growth_exhaust', 'bonus_pop_growth_mult = 1.00')
